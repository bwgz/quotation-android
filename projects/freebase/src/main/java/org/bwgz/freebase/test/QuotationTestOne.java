...
class QuotationResponse extends MQLMultipleResultResponse<Quotation> {
}

public class QuotationTestOne {
	...
	public static void main(String[] args) {
		...
		map.put(Quotation.class, directives);

		MQLQueryBuilder builder = new MQLQueryBuilder(MQLQueryBuilder.PROPERTY_PRETTY);
		String mql = builder.createQuery(Quotation[].class, map);
		...
		for(;;) {
			try {
		    	...
		    	ResponseEntity<QuotationResponse> entity = restTemplate.getForEntity(uri, QuotationResponse.class);
		        
		    	QuotationResponse response = entity.getBody();
		    	Quotation[] quotations = response.getResult();
		        for (Quotation q : quotations) {
		        	System.out.printf("quotation: %s\n", q.getName());
		        }
		        System.out.printf("returned: %s  cursor: %s\n", quotations.length, response.getCursor());
