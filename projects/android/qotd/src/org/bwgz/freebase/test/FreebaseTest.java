...
import org.bwgz.freebase.query.Util;
...
public class FreebaseTest<T> {
	...
	public static void main(String[] args) {
		FreebaseTest<Person> test1 = new FreebaseTest<Person>();
		test1.query(personQuery, Person.class);

		FreebaseTest<Quotation> test2 = new FreebaseTest<Quotation>();
		test2.query(quotationQuery, Quotation.class);
		
		MQLQueryBuilder builder = new MQLQueryBuilder();
		Quotation quotation = new Quotation();
		
		quotation.setId("/en/first_thing_we_do_lets_kill_all_the_lawyers");
		String query = builder.createQuery(Quotation[].class, null, quotation);
		
		FreebaseTest<Quotation> test3 = new FreebaseTest<Quotation>();
		test3.query(query, Quotation.class);
